...
switch Action
    case 'init'
        ...
        BpodSystem.GUIHandles.FutureTrialLine = line([Xdata,Xdata],[Ydata,Ydata],'LineStyle','none', 'Marker','.','MarkerEdge','k','MarkerFace','[1 1 1]', 'MarkerSize',10);
        BpodSystem.GUIHandles.CurrentTrialCircle = line([0,0],[0,0], 'LineStyle','none','Marker','+','MarkerEdge','k','MarkerFace',[1 1 1], 'MarkerSize',6);
        BpodSystem.GUIHandles.UnpunishedErrorLine = line([0,0],[0,0], 'LineStyle','none','Marker','o','MarkerEdge','g','MarkerFace',[1 1 1], 'MarkerSize',6);
        BpodSystem.GUIHandles.PunishedErrorLine = line([0,0],[0,0], 'LineStyle','none','Marker','x','MarkerEdge','r','MarkerFace','r', 'MarkerSize',6);
        ...
        if verLessThan('matlab','8.0') % Use optimal split function if possible
            BpodSystem.GUIHandles.TTOP_Ylabel = Split(num2str(4:-1:-1));
        else
            BpodSystem.GUIHandles.TTOP_Ylabel = strsplit(num2str(4:-1:-1));
        end
        set(AxesHandle,'TickDir', 'out','YLim', [-4-.5, -.5], 'YTick', -4:1:-1,'YTickLabel', {'ProbeNoGO' 'ProbeGO' 'NoGo' 'Go'}, 'FontSize', 12);
    ...
    case 'update'
        ...
        MaxTrialType = max(TrialTypeList);
        set(AxesHandle,'YLim',[-4-.5, -.5], 'YTick', -4:1:-1,'YTickLabel', {'ProbeNoGO' 'ProbeGO' 'NoGo' 'Go'});
        if CurrentTrial<1
            CurrentTrial = 1;
        end
...
% FractionWindowStickpoint = 1; % After this fraction of visible trials, the trial position in the window "sticks" and the window begins to slide through trials.
% mn = max(round(CurrentTrial - FractionWindowStickpoint*nTrialsToShow),1);
% mx = mn + nTrialsToShow - 10;
% % tickLabels = sprintfc('%d',(mn-1:10:mx));
% % set(AxesHandle,'XtickLabel', tickLabels);
% % if CurrentTrial > nTrialsToShow
% %     xticks(CurrentTrial:5:nTrialsToShow+nTrialsToShow);
% % end
